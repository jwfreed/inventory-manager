...
test('quantity conservation reports the exact item-location drift after projection tampering', async () => {
  ...
  try {
    ...
    // BREAK INVARIANT inside a rollback-scoped transaction: drift the projected
    // on-hand balance away from the ledger-derived quantity.
    await client.query(
    ...
    const mismatches = await harness.findQuantityConservationMismatches({ client });
