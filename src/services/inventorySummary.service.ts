...
export async function assertItemExists(id: string) {
  const res = await query('SELECT 1 FROM items WHERE id = $1', [id]);
  return res.rowCount > 0;
}

export async function assertLocationExists(id: string) {
  const res = await query('SELECT 1 FROM locations WHERE id = $1', [id]);
  return res.rowCount > 0;
}

export async function getItemInventorySummary(itemId: string) {
...
export async function getLocationInventorySummary(locationId: string) {
  const { rows } = await query(
