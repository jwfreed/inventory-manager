...
const AUDIT_TABLE = 'migration_orphan_parent_fix_1772500001200';
...
export async function up(pgm: MigrationBuilder): Promise<void> {
  ...
  pgm.sql(`
    ...
    BEGIN
      ...
      single_wh AS (
        SELECT l.tenant_id, l.id AS warehouse_id
          FROM locations l
          JOIN single_wh_tenants t USING (tenant_id)
      ...
      RAISE NOTICE 'orphan_fix_retry: before=%, updated=%, remaining=%, tenants=%',
      ...
      LOOP
        RAISE NOTICE 'orphan_fix_retry_remaining: tenant_id=% count=%', r.tenant_id, r.remaining_orphans;
      END LOOP;
    END $$;
  `);
}
