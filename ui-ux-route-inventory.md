...
| `/qc/receipts/:receiptId` | `inventory:qc:write` | QC | Accept, hold, reject, resolve held quantity | `QcClassificationPage`, `QcDetailPanel`, `BulkOperationsBar`, `ReceivingContext` | High |
| `/receiving/qc` | `inventory:qc:write` | QC | Legacy/query-param QC path | `QcClassificationPage` | High |
| `/qc-events/:qcEventId` | `inventory:qc:write` | Traceability / QC | Inspect a QC event | `QcEventDetailPage` | Medium |
| `/receiving/putaway` | `inventory:putaway:write` | Putaway | Create and post putaway for accepted stock | `PutawayPlanningPage`, `DraggablePutawayLinesEditor`, `PutawaySummaryTable`, `ReceivingContext` | High |
| `/atp` | `inventory:read` | Inventory visibility / allocation support | Query available-to-promise | `AtpQueryPage`, `AtpResultsTable` | Medium |
| `/lpns` | `inventory:read` | Inventory visibility / trace support | View license plates | `LicensePlatesPage` | Medium |
| `/inventory-transfers/new` | `inventory:transfers:write` | Transfers | Post direct location-to-location transfer | `InventoryTransferCreatePage`, `InventoryTransferForm`, `TransferOperationPanel` | High |
...
| `/inventory-counts/:id` | `inventory:read`; edit by `inventory:counts:write` | Cycle counting / reconciliation | Save draft and post adjustment | `InventoryCountDetailPage`, `InventoryCountForm` | Medium |
...
| `/reservations` | `outbound:read` | Allocation | Allocate or cancel reservations | `ReservationsListPage`, `reservationActionPolicy` | Medium |
| `/reservations/:id` | `outbound:read` | Allocation | Allocate, cancel, fulfill reservation | `ReservationDetailPage`, `reservationActionPolicy` | Medium |
...
| `/sales-orders/:id` | `outbound:read` | Pick / pack / ship | Create shipment document from order lines | `SalesOrderDetailPage` | High |
