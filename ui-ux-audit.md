...
Audit scope: source inspection of React UI routes, shared UI components, domain invariant documentation, and backend/API enforcement paths. No visual screenshot claims are made.

Severity distinguishes UI-driven operational risk from backend data-integrity risk. Where backend services already fail closed, the finding remains valid as a usability or operator-error risk but is not treated as direct inventory corruption risk.
...
- Evidence (file path, component, function): `ui/src/features/receiving/pages/PutawayPlanningPage.tsx`, keyboard shortcut lines 88-95; post button lines 437-446; completion copy lines 137-140; backend posting checks in `src/services/putaways.service.ts` lines 629-709 and route error mapping in `src/routes/putaways.routes.ts` lines 138-203.
- Backend Invariant Status: Enforced. Backend validates QC availability, accepted quantity limits, source stock, voided receipts, negative override policy, idempotency, and replay integrity.
- Operational impact: A wrong destination can still become operational truth if the operator posts before reviewing all source/destination movements. Backend prevents impossible or over-quantity stock states, so the remaining risk is UI-driven misposting rather than backend data corruption.
...
- Evidence (file path, component, function): `ui/src/features/inventory/pages/InventoryTransferCreatePage.tsx`, `validateTransferForm` lines 97-115 and mutation lines 67-78; `ui/src/features/inventory/components/InventoryTransferForm.tsx`, fields and submit button lines 45-130; backend transfer validation in `src/routes/transfers.routes.ts` lines 90-156 and `src/services/stockValidation.service.ts` lines 175-228.
- Backend Invariant Status: Enforced. Backend validates source/destination, positive quantity, stock availability, negative override policy, ATP locks, idempotency, and replay integrity.
- Operational impact: Operators must guess whether the source can support the move and learn from backend failure. In a warehouse, this increases trial-and-error and can cause wrong source selection under time pressure, even though backend rejects unavailable transfers.
...
- Finding: The ATP page description says "on-hand minus reservations"; it does not mention allocated quantity, sellable-location filtering, holds/quarantine, blocked locations, in-transit, or other unavailable states.
- Evidence (file path, component, function): `ui/src/features/inventory/pages/AtpQueryPage.tsx`, page description lines 45-49; ATP columns lines 140-150 and results lines 172-180; backend ATP service reads `inventory_available_location_sellable_v` and includes allocated quantity in `src/services/atp.service.ts` lines 75-155.
- Backend Invariant Status: Enforced. Backend ATP calculation uses the sellable availability view and includes allocated quantities; the issue is incomplete UI explanation and omitted visible columns.
- Operational impact: Users may equate ATP with a simple arithmetic shortcut and make allocation decisions that ignore allocated or excluded stock states. Backend calculation remains authoritative, but the UI copy teaches an unsafe mental model.
- Workflow affected: Inventory visibility, allocation, pick / pack / ship.
- Severity (Critical / High / Medium / Low): Medium.
- Recommendation: Change explanatory copy to "ledger-backed sellable availability after reservations, allocations, and excluded states"; show allocated and excluded-state explanation or columns when API data supports it.
- Effort (S/M/L): S.
...
## Finding 7: Cycle count posting does not expose line-level variance consequences before adjustment

- Finding: Count lines accept optional reason codes during draft entry, and the post confirmation shows only aggregate variance count and total absolute variance. The UI does not show per-line book-vs-count adjustment consequences before posting.
- Evidence (file path, component, function): `ui/src/features/inventory/components/InventoryCountForm.tsx`, reason code field lines 200-206; `ui/src/features/inventory/pages/InventoryCountDetailPage.tsx`, save payload maps empty reason to undefined lines 126-137; post modal aggregate-only copy lines 347-368; backend reason and cost enforcement in `src/services/counts.service.ts` lines 910-935 and 1472-1485.
- Backend Invariant Status: Enforced. Backend rejects non-zero count variances without reason code, requires unit cost for positive variances, applies reconciliation thresholds, and validates stock for negative variances.
- Operational impact: Operators cannot review which items/locations will be adjusted from the final confirmation. Backend prevents reasonless variance posting, so this is a confirmation and cognitive-load risk rather than an unsupported adjustment path.
- Workflow affected: Cycle counting / reconciliation, inventory visibility.
- Severity (Critical / High / Medium / Low): Medium.
- Recommendation: Show a variance review table before post with item, location, UOM, book quantity, counted quantity, delta, adjustment direction, unit cost requirement, and reason for every variance line.
- Effort (S/M/L): M.
- Verification method: UI test that post confirm lists variance lines and exposes missing backend-required fields before the backend rejection path; API test remains as guard for reason enforcement.

## Finding 8: On-hand import can be applied for tracked items without lot/serial support or final review
...
- Evidence (file path, component, function): `ui/src/features/orderToCash/pages/ReservationsListPage.tsx`, disabled allocate/cancel buttons with `title` lines 228-244; `ui/src/features/orderToCash/pages/ReservationDetailPage.tsx`, action buttons with `title` lines 286-320; policy text in `ui/src/features/orderToCash/lib/reservationActionPolicy.ts` lines 20-60.
- Backend Invariant Status: Enforced. Backend reservation transitions reject invalid allocate/cancel/fulfill states and require idempotency keys.
- Operational impact: Operators can see unavailable actions but may not understand the required next step, increasing dead-end behavior and support escalation. Backend protects invalid transitions, so this is an accessibility and recovery issue.
...
- Backend Invariant Status: Enforced. Backend invariants are not directly affected by table affordances; this is a UI discoverability and exception-scanning issue.
...
- Evidence (file path, component, function): `ui/src/features/purchaseOrders/pages/PurchaseOrderCreatePage.tsx`, create submit button lines 337-343; `PurchaseOrderDetailPage` confirmation alert lines 486-507; backend submit-readiness validation in `src/services/purchaseOrders.service.ts` lines 215-234.
- Backend Invariant Status: Enforced. Backend validates PO readiness before creating submitted/approved POs.
...
- Purchasing / PO creation: Finding 12 and route inventory.
