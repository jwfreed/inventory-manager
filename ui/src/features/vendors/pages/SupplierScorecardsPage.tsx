...
export function SupplierScorecardsPage() {
  ...
  const detailQuery = useQuery({
    queryKey: ['supplier-scorecards', vendorId],
    queryFn: () => getSupplierScorecard(vendorId ?? ''),
  ...
  const allQuery = useQuery({
    ...
    enabled: !vendorId && activeTab === 'all',
  ...
  const deliveryQuery = useQuery({
    ...
    enabled: !vendorId && activeTab === 'delivery',
    staleTime: 300_000,
  })
  ...
  const qualityQuery = useQuery({
    ...
    enabled: !vendorId && activeTab === 'quality',
    staleTime: 300_000,
  })
  ...
  const issuesQuery = useQuery({
    ...
    enabled: !vendorId && activeTab === 'issues',
    staleTime: 300_000,
  })
  ...
  const visibleScorecards = vendorId
    ? detailQuery.data?.data
      ? [detailQuery.data.data]
      : []
    : activeQuery.data?.data ?? []
  const visibleQuery = vendorId ? detailQuery : activeQuery
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          {vendorId
            ? 'Review supplier performance across delivery and quality metrics'
            : 'Track supplier performance across delivery and quality metrics'}
        </p>
        {vendorId && (
          <Link to="/supplier-scorecards" className="mt-2 inline-block text-sm font-medium text-blue-600 hover:text-blue-700">
            Back to all supplier scorecards
          </Link>
        )}
      </div>
      ...
      <Section>
        {visibleQuery.isLoading ? (
          ...
          <ScorecardTable
            ...
            highlightMetric={
              vendorId ? undefined : activeTab === 'delivery' ? 'delivery' : activeTab === 'quality' ? 'quality' : undefined
            }
          />
        )}
      </Section>
